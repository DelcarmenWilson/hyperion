...
export const replaceScript = (
  ...
  const license =
    licenses?.find((e) => e.state == lead.state)?.licenseNumber || NS;
  const { defaultNumber, firstName, dateOfBirth, address, state, smoker } =
    lead;
  // USER INFO
  script = script
    .replaceAll(Keywords.MY_FIRST_NAME, agentFirstName)
