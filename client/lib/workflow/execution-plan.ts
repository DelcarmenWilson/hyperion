...
import { AppNode,AppNodeMissingInputs } from "@/types/workflow/app-node";
import {
...
const getInvalidInputs = (
  ...
  for (const input of inputs) {
    ...
    if (requiredInputProvidedByVisitedOutput) {
      // the input is requiered and we have a valid value for it
      // provided by a task that is already palnned
      continue;
      ...
      if (inputLinkedToOutput && planned.has(inputLinkedToOutput.source)) {
        // The output is providing a value to the input: the input i fine
        continue;
      }
    }
