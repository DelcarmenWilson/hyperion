...
import { funnelUpsertById } from "@/actions/funnel";
...
const FunnelForm: React.FC<CreateFunnelProps> = ({ defaultData }) => {
  const { setClose } = useModal();
  const router = useRouter();
  const form = useForm<CreateFunnelFormSchemaType>({
    mode: "onChange",
  ...
  const onSubmit = async (values: z.infer<typeof CreateFunnelFormSchema>) => {
    const response = await funnelUpsertById(
    ...
    else toast.error("Could not save funnel details");
    setClose();
    router.refresh();
  };
  return (
