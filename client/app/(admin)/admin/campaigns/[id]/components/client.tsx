"use client";

import { MessageSquare } from "lucide-react";

import { Card, CardContent } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { useCampaignData } from "@/hooks/use-campaigns";

export const CampaignClient = ({ campaingId }: { campaingId: string }) => {
  const { campaign } = useCampaignData(campaingId);
  return (
    <Card className="flex flex-col flex-1 relative overflow-hidden">
      <div className="flex items-center mb-2">
        <div className="flex items-center gap-2">
          <div className="bg-accent p-4 rounded-br-lg">
            <MessageSquare className="h-5 w-5 text-primary" />
          </div>
          <p>
            Campaign <span>{campaign?.name}</span>
          </p>
        </div>
      </div>
      <Separator />
      <CardContent className="flex flex-col flex-1 p-2 gap-2 overflow-hidden"></CardContent>
    </Card>
  );
};
