...
import { convertAppointments } from "@/formulas/inital-data";
import { initialAppointments } from "@/actions/initial-data";
...
export const AppointmentsBox = () => {
  ...
  const onFileUploaded = (e: any) => {
    Papa.parse(e.target.files[0], {
      ...
      complete: function (result: any) {
        const mapped: Appointment[] = convertAppointments(result);
  ...
  const onImport = () => {
    startTransition(() => {
      initialAppointments(appointments).then((data) => {
        if (data?.success) {
          onCancel();
  ...
  return (
    <div className="h-[500px] flex flex-col gap-2 p-3 bg-background">
      <div className="flex flex-col justify-center items-center">
        <h3 className="text-2xl font-semibold py-2">Import Appointments</h3>
